...
4. **The core stays framework-agnostic.** The CSS and JavaScript carry no dependency on Python or
   Jinja; the macros are an optional layer on top. A change that makes the plain assets unusable
   without the Python package does not belong in the core.
